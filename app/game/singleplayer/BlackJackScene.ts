...
import { StatisticsEmitter } from "../shared/StatisticsEmitter";
...
export class BlackjackScene extends Phaser.Scene {
	...
	*/
    ...
    private createUI() {
        ...
        // Title
        this.add
            .text(cx, 60, "BLACKJACK", { fontSize: "64px", fontStyle: "bold" })
            .setOrigin(0.5);

        // Coins Display
        this.coinsText = this.add
            .text(cx, 130, `Fichas: ${this.playerCoins}`, {
                fontSize: "40px",
                color: "#ffff00",
            })
            .setOrigin(0.5);
        ...
        this.dealerScoreText = this.add
            .text(cx, 400, "Dealer: 0", { fontSize: "40px" })
            .setOrigin(0.5);
        this.playerScoreText = this.add
            .text(cx, 700, "Jogador: 0", { fontSize: "40px" })
            .setOrigin(0.5);
    ...
    private createButtons(cx: number) {
        ...
        this.hitBtn = new BlackjackButton(
            this,
            cx - 300,
            btnY,
            "PUXAR",
            GAME_CONFIG.colors.buttonHit,
            () => this.handleHit(),
            "btn_blue"
        );

        this.standBtn = new BlackjackButton(
            this,
            cx,
            btnY,
            "PARAR",
            GAME_CONFIG.colors.buttonStand,
            () => this.handleStand(),
            "btn_red"
        );

        this.dealBtn = new BlackjackButton(
            this,
            cx,
            btnY,
            "INICIAR",
            GAME_CONFIG.colors.buttonDeal,
            () => this.startPlayingPhase(),
            "btn_green"
        );

        this.restartBtn = new BlackjackButton(
            this,
            cx,
            btnY,
            "CONTINUAR",
    ...
    private renderCard(card: CardData, isPlayer: boolean, isHidden: boolean) {
        const container = this.add.container(
            GAME_CONFIG.pos.deck.x,
            GAME_CONFIG.pos.deck.y
        );
        ...
        back.fillRoundedRect(
            -GAME_CONFIG.cardWidth / 2,
            -GAME_CONFIG.cardHeight / 2,
    ...
    private repositionHand(hand: CardData[], isPlayer: boolean) {
        const visibleCards = hand.filter((card) =>
            this.cardContainers.has(card)
        );
        ...
        const yPos = isPlayer
            ? GAME_CONFIG.pos.playerHandY
            : GAME_CONFIG.pos.dealerHandY;
    ...
    private revealDealerCard() {
        ...
        this.tweens.add({
            ...
            duration: 150,
            onComplete: () => {
                if (back && "setVisible" in back)
                    (back as any).setVisible(false);
    ...
    private handleHit() {
        ...
        this.time.delayedCall(500, () => {
            ...
            console.log(
                `Hit Update - Win Chance: ${(this.winChance * 100).toFixed(
                    1
                )}% | Chance to 21: ${(this.chanceTo21OnNextDraw * 100).toFixed(
                    1
                )}%`
            );
            this.emitStatistics();
    ...
    private calculateHypergeometric(
        ...
        return (
            (waysToChooseSuccesses * waysToChooseFailures) / totalWaysToChoose
        );
    }
    ...
    private endGame(message: string, winStatus: boolean | null) {
        ...
        // Emit statistics after updating win/loss history
        this.emitStatistics();
    }
    ...
    private updateStatsAfterDeal() {
        ...
        console.log(
            `Stats Update - Win Chance: ${(this.winChance * 100).toFixed(
                1
            )}% | Chance to 21: ${(this.chanceTo21OnNextDraw * 100).toFixed(
                1
            )}%`
    ...
    private updatePerfectPlayStats(action: "hit" | "stand") {
        ...
        console.log(
            `Move: ${action.toUpperCase()} | Optimal: ${optimal.toUpperCase()} | Perfect%: ${(
                this.perfectPlayPercent * 100
            ).toFixed(1)}%`
        );
    }
    ...
    private updateLuckStats(winStatus: boolean | null) {
        ...
        console.log(
            `Luck Update: Result(${resultScore}) - Chance(${this.winChanceAtStand.toFixed(
                2
            )}) = ${luckImpact.toFixed(
                2
            )} | Total Luck: ${this.luckFactor.toFixed(2)}`
        );
    }
    ...
    private trackAction(action: "hit" | "stand") {
        ...
        this.riskAverage =
            this.riskHistory.reduce((a, b) => a + b, 0) /
    ...
    private calculateChanceTo21(): number {
        ...
        return this.calculateHypergeometric(
            totalCards,
            favorableCardsCount,
            1,
            1
    ...
    private emitStatistics() {
        ...
        // Emit bust probability
        const bustProbability = this.calculateBustProbability();
        emitter.emit("bustProbability", { bustProbability });

        // Emit win chance
        emitter.emit("winChance", { winChance: this.winChance });

        // Emit win chance history
        emitter.emit("winChanceHistory", {
            winChanceHistory: this.winChanceHistory,
        });

        // Emit risk history
        emitter.emit("riskHistory", {
            riskHistory: this.riskHistory,
        });

        // Emit win/loss history
        emitter.emit("winLossHistory", {
            winLossHistory: this.winLossHistory,
        });

        // Emit perfect play stats
        emitter.emit("perfectPlay", {
            perfectPlayPercent: this.perfectPlayPercent,
            totalDecisions: this.totalDecisions,
        });

        // Emit luck stats
        emitter.emit("luck", {
            luckHistory: this.luckHistory,
