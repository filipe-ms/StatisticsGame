...
export class BlackjackScene extends Phaser.Scene {
	...
	*/
	...
	private calculateWinProbability(simulations = 1000): number {
		...
		// Check if the dealer has a hidden card (Playing state + has 2+ cards)
		const hasHiddenCard = this.currentState === GameState.Playing && this.dealerHand.length >= 2;

		for (let i = 0; i < simulations; i++) {
			// 1. Create the pool of unknown cards.
			const simDeck = [...this.deck];

			// 2. Prepare the simulated dealer hand.
			const simDealerHand = [...this.dealerHand];

			if (hasHiddenCard) {
				simDeck.push(this.dealerHand[0]);
				simDealerHand.shift();
			}
			...
			if (hasHiddenCard) {
				simDealerHand.unshift(simDeck.pop()!);
			}
